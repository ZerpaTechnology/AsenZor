	...
	def cnt(p,m):
		...
		if "vista" in p:
			...
			for elem in ["vistas"]:
					...
					sub=open(p["base_root"]+"../user/"+roots.controles_folder+roots.sub_folder+elem+".py","r")
					script=sub.read()
					sub.close()
					exec(script)
			try:
				if "admin" in p and zred.normalizar(p["admin"])==True:
					if p["vista"] in settings.config.vistas:
					...
					elif p["vista"] in settings.config.vistas_login and logueado==True:
						m["servir"](p["vista"],p["base_root"]+roots.vistas_folder,p["base_root"]+roots.templates_url,data=data)
					else:
						m["servir"]("error404",p["base_root"]+roots.vistas_folder,p["base_root"]+roots.templates_url,data=data)
