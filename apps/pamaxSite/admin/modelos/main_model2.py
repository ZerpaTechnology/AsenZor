...
try:
	class model(Model):
		...
		#===================================================================
		#Cuerpo del modelo
		def registrarUsuario(self,username,email,password,ips):
			import datetime
			...
			if self.request():
				...
				self.db('users').insertar(username, 'mailto:'+email, 'password:'+password, token,ips, True)
		...
		def login(self,usuario,password,ip):
			...
			if self.request():
				...
				if self.db("users").obtenerFilasValores(usuario)!=[]:
					if  self.db("users").obtenerFilasValores(usuario)[5]==False:
		...
		def closeSession(self,token,ip):
			...
			if self.request():
				...
				if filas[3]==token and ip in self.db("users").obtenerFilasValores(token)[4]:
		...
		def consultarLogin(self,token,ip):
			try:	
				...
				if self.request():
					...
					if self.db.tablas["users"][0][3].valor==token:


						if token== self.db("users").obtenerFilasValores(token)[3] and ip in self.db("users").obtenerFilasValores(token)[4]:
							
							if self.db("users").obtenerFilasValores(token)[5]==True:
