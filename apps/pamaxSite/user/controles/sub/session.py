...
try:
	...
	zurl=zred.redirecter(config.base_url,"pamaxSite","index")
	for elem in data:
		keys.append(elem)
	if data["action"].value=="sing_up":

		l=[]
		l2=["username","email","password"]
		for elem in l2:
			if elem not in keys:
		...
		if l==[]:
			...
			modelo=model(p["base_root"]+"../admin/"+settings.config.model_folder+settings.config.dbs[0],p["base_root"]+"../admin/"+settings.config.resquest_folder,data["username"].value,debug=False)
			
			token=modelo.registrarUsuario(data["username"].value,data["email"].value,data["password"].value,zu.randomString(),[p["user_ip"]])
			if token!=False:
				print "logueado"
				zurl("index")
	...
	if data["action"].value=="confirmarUser":
					main_model2.registrarUsuario(p["cod"])

	if data["action"].value=="closeSession":
		respuesta=main_model2.closeSession(p["token"],ip)
		if respuesta==True:
