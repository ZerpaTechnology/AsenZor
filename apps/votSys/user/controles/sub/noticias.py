...
try:
	...
	if data["action"].value=="publicarNoticia":
		...
		if l==[]:
			...
			modelo.publicarNoticia(data[""].value,data["password"].value,data["email"].value,data["name"].value,data["lastname"].value,int(data["rank"].value),data["avatar"].value,int(data["department_id"].value),int(data["position_id"].value))
